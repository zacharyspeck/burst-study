...
Expected: `325 passed, 156 skipped`, then the resolved config printed, exit
status 0, and
...
The 156 skips are the tests that need torch or `transformers`, which the install
...
```bash
pip install -e ".[dev]"            # config work — no ML stack, 325 tests run
