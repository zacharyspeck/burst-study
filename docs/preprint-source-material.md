# Source material for the preprint

**Not the preprint.** `docs/preprint.md` is the paper: a NeurIPS-style skeleton of
section headings, tables and bracketed placeholders, targeted at ATTRIB 2026
(3--6 pages plus unlimited appendix).

This file is the **prose the placeholders are to be filled from** --- every result
written out at length, with the reasoning, the caveats and the numbers in
context. Nothing here is final text; it is the long-form version to cut down,
and the place to check what a number means before quoting it.

Every figure in it is derived from `docs/measurements/`, not typed by hand. See
`docs/measurements/2026-08-10-full-study-results.md` for the compact record of
the same results.
