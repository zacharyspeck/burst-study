...
```bash
python -m pytest -q                    # 1036 tests; see below on pass/skip
python scripts/generate_overrides.py --check   # expect 70 ok, 0 missing, 0 mismatched
```

**Compare the total, not the pass count.** 1036 tests collect everywhere, but
...
with a corpus built this is **662 passed, 228 skipped** without torch and
**1036 passed, 0 skipped** with it; the gpmoo figures are one corpus-test skip
