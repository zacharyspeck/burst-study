...
_BLOCKED = (
    ...
    "{what} requires scripts/canonicalize.py, which is Conv1D-only by "
    "deliberate decision (its tripwire REFUSES a transposed model rather than "
    "silently addressing the wrong axis). Which layout the study trains is "
    "undecided in this repo: the intent is HF GPT-2, but that swap has not "
    "landed and nothing here may assume it has.\n"
    "Building this now means building against a model that does not exist.\n"
